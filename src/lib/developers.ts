...
export async function loadDeveloper(login: string): Promise<DeveloperExperience | null> {
  ...
  if (mode !== "recorded") try { const live = await getLive(login); profile = live.profile; events = live.events; capturedAt = new Date().toISOString(); source = "live"; } catch { /* recorded source stays visible and explicit */ }
